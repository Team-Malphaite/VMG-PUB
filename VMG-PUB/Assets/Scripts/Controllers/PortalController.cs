...
public class PortalController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        go = GameObject.Find("@Player").transform;

        if (Managers.Scene._portalCheck)
        {
            if (PlayerController.Instance._mode == PlayerController.modeState.Square)
            {
                PlayerController.Instance._mode = PlayerController.modeState.Voting;
                Managers.Scene.LoadScene(Define.Scene.Voting);
                // go.position = new Vector3(0.18f, 0.27f, -9f);
                Managers.Scene._portalCheck = false;
            }

            else if (PlayerController.Instance._mode == PlayerController.modeState.Voting)
            {
    ...
    void OnTriggerEnter(Collider col) {
        if (col.gameObject.tag == "Player") {
            if (PlayerController.Instance.photonView.IsMine)
            {
                ...
                if (PlayerController.Instance._mode == PlayerController.modeState.Voting)
                {
                    string title = "이동";
